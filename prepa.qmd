...
Reproducimos ahora la misma lógica para la obra "El cid de las niñas" de Leandro Fernández de Moratín. Como en el caso de "Tres sombreros de copa", la obra se divide en actos y escenas. El objetivo es crear una base de datos con los diálogos, identificar los puntos de corte y crear las redes de diálogos dirigidas y no dirigidas.
...
texto <- read_lines("https://raw.githubusercontent.com/rodrodr/filoR/refs/heads/master/cid_ninas.txt")

# Crea una base de datos para
# almacenar los diálogos
cid <- data.frame(Acto=character(), 
                  Escena=character(), 
                  Personaje=character(), 
                  Texto=character(), 
...
for (linea in texto) {
  ...
  if (stri_detect_regex(linea, "^([A-ZÁÉÍÓÚÑÜ ]+)(?:\\.|\\.-)\\s+(.*)$")) {
    ...
    cid <- add_row(cid, Acto = acto, Escena = escena,
                        Personaje = personaje, Texto = texto_dialogo)
  }
}


cid$orden <- 1:nrow(cid)

cid$Personaje[cid$Personaje=="Diego"] <- "Don Diego"
cid$Personaje[cid$Personaje=="D Carlos"] <- "Don Carlos"

# Vemos el resultado
reactable(cid, resizable = T, wrap = T)

```



Ahora repetimos lo que hicimos con "Tres sombreros de copa" para crear redes de diálogo. Repetimos el mismo código, pero ahora con la nueva base de diálogos de "El cid de las niñas".
...
# datos para almacenar el personaje
# que será el receptor de la respuesta
cid$personaje_A <- cid$Personaje
cid$personaje_B <- NA

# Para cada diálogo de la pieza
for(i in 1:(nrow(cid)-1)){
  
  # Si es la introducción del capitulo o de la obra,
  # se pasa al siguiente diálogo
  if(cid$personaje_A[i]=="Introducción") next
  ...
  if(cid$corte[i]==1){
    ...
    # Si el personaje anterior es la introducción
    # se pasa al siguiente diálogo
    if(cid$personaje_A[i-1]=="Introducción") next
    
    # Atribuye el personaje de destino del diálogo
    # como en personaje anterior (respuesta final)
    cid$personaje_B[i] <- cid$personaje_A[i-1]
...
# Elimina los espacios en blanco al 
# final de los nombres de los personajes
cid$personaje_A <- trimws(cid$personaje_A)
cid$personaje_B <- trimws(cid$personaje_B)
...
# Separa solo los pares de personaje
# en diálogo
res <- cid[,c("personaje_A","personaje_B")]
res <- res[! is.na(res$personaje_B),]
...
# ordenador donde podáis rescatar
# los datos luego:
# "C:/FiloR/cid_ninas.RData", 
# por ejemplo
save(cid_d, 
     cid_rd, 
     cid_rd, 
     file="../textos/cid_ninas.RData")
